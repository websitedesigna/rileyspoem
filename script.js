// Riley Oliver Poetry Collection - Complete Application
class PoetryApp {
    constructor() {
...
still choosing life after all these years`
 ...
 setupSearch() {
    ...
    filterAndDisplayPoems() {
        ...
        const filteredPoems = this.getFilteredPoems();
        this.updateActiveFilter(this.currentFilter);
        this.updateGenreCounts();
        if (filteredPoems.length === 0) {
            noResults.style.display = 'block';
            return;
    ...
    createPoemCard(poem) {
        ...
        readMoreBtn.addEventListener('click', (e) => {
            ...
            this.resetFilters();
            this.updateActiveFilter(primaryGenre);
        });
    ...
    updateGenreCounts() {
        ...
        document.querySelectorAll('.genre-card').forEach(card => {
            ...
            if (countElement && genreCounts.hasOwnProperty(genre)) {
                ...
                countElement.textContent = `${count} poem${count !== 1 ? 's' : ''}`;
            }
        });
    }
    ...
    setupIntersectionObserver() {
        ...
        const observeElements = () => {
            document.querySelectorAll('.genre-card:not(.animate-in)').forEach(card => {
        ...
        mutationObserver.observe(document.getElementById('featuredPoems'), {
            childList: true
        });
    }
